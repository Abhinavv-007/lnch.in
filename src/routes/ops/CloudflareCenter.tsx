        ...
        <ul className="divide-y divide-ink-600/40 text-sm">
          {(data?.traffic?.zones ?? []).map((zone) => (
            <li key={zone.zoneId} className="grid gap-2 py-3 md:grid-cols-[1.3fr_repeat(5,minmax(0,1fr))] md:items-center">
              <span>
                <span className="text-ink-100">{zone.zoneName}</span>{" "}
                <span className="text-xs text-ink-300">{zone.since} → {zone.until}</span>
              </span>
              <span className="text-ink-300">req <b className="text-ink-100">{compact.format(zone.totals.requests)}</b></span>
              <span className="text-ink-300">views <b className="text-ink-100">{compact.format(zone.totals.pageViews)}</b></span>
              <span className="text-ink-300">unique <b className="text-ink-100">{compact.format(zone.totals.uniqueVisitors)}</b></span>
              <span className="text-ink-300">data <b className="text-ink-100">{formatBytes(zone.totals.bytes)}</b></span>
